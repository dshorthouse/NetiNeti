...
class NetiNetiTrain:
        ...
        for p,q,r in neg_trigrams:
            ...
            if(p[0].isupper() and p[1:].islower() and q.islower()):
                ...
                if(r.islower()):
                    ...
                    #print "building neg features... ", tgc*100/float(self._num_neg_tg),"% done"
                    #print tgc*100/float(self._num_neg_tg)
    ...
    def taxon_features(self,token,context_array,index,span):
        ...
        for c in range(context_span):
            ...
            #features[str(c+1)+"_context"] = self._populateFeatures(context_array,index+span+c+1,0,"end",features,str(c+1)+"_context").strip()
            ...
            if(index+c-context_span < 0):
                ...
                #features[str(c-context_span)+"_context"] = self._populateFeatures(context_array,index+c-context_span,0,"end",features,str(c-context_span)+"_context").strip()
...
##                else:
##                        features["pos_tag"+str(-1)+"_context"] = nltk.pos_tag([features[str(-1)+"_context"]])[0][1]
##
##                if( R =="Null"):
##                        features["pos_tag"+str(1)+"_context"] = "UKNWN"
##                else:
...
##
    ...
    def _buildFeatures(self,featuresets):
        ...
        if(self.learning_algo =="NB"):
            #WNB = nltk.classify.weka.WekaClassifier.train("Naive_Bayes_weka",featuresets,"naivebayes")
...
class NameFinder():
    def __init__(self,modelObject,e_list='data/new-list.txt'):
        reml = {}
        elist = open(os.path.dirname(__file__) + "/"  + e_list).read().split("\n")
        for a in elist:
            reml[a] = 1
        self._remlist = reml
    ...
    def _isGood3(self,a,b,c):
        ...
        if(len(a) >1 and len(b) > 1 and len(c) > 1):
            ...
            if(b_par_exp):
                s2 = b[1].isupper() and ((b[2] =="." and len(b) ==4) or b[2:-1].islower() and b[2:-1].isalpha()) and b[-1]!="."
                return(s1 and self._hCheck(c) and s2 and (a[0].isupper() and ((a[1] =="." and len(a)==2) or a[1:].islower() and a.isalpha())))
            else:
        ...
        if(len(token) ==2):
            ...
            for a,b,c in tgr:
                ...
                if(self._isGood3(p,q,r)):
                    ...
                    if(self._hCheck(p) and p[0].isupper() and self._remDot(p).isalpha()):
                        start,end = self._getOffsets(oh,icount,a,"","")
                        offset_list.append((start,end))
                        nms.append(self._remDot(p))
